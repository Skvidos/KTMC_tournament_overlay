...
document.addEventListener('DOMContentLoaded', (event) => {
	...
	document.querySelectorAll('.player-item').forEach(item => {
		item.addEventListener('click', (event) => {
			selectedPlayer = event.currentTarget;
		});
	});

	document.querySelectorAll('.group-teammate').forEach(item => {
		item.addEventListener('click', (event) => {
			if (selectedPlayer) {
				const playerSeed = selectedPlayer.querySelector('.player-seed').textContent;
				const playerName = selectedPlayer.querySelector('.player-name').textContent;

				const groupTeammateSeed = event.currentTarget.querySelector('.group-teammate_seed');
				const groupTeammateName = event.currentTarget.querySelector('.group-teammate_name');

				groupTeammateSeed.textContent = playerSeed;
				groupTeammateName.textContent = playerName;

				selectedPlayer.classList.add('selected');

				lastAction = {
					player: selectedPlayer,
					groupTeammate: event.currentTarget,
	...
	document.getElementById('undo-button').addEventListener('click', (event) => {
		if (lastAction) {
			...
			const groupTeammate = lastAction.groupTeammate;

			const groupTeammateSeed = groupTeammate.querySelector('.group-teammate_seed');
			const groupTeammateName = groupTeammate.querySelector('.group-teammate_name');

			groupTeammateSeed.textContent = '-';
			groupTeammateName.textContent = '-';
