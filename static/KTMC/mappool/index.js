...
let lastState;
const gameplay_scene_name = 'Match';
...
async function setupBeatmaps() {
    ...
    for (const beatmap of maps) {
        ...
        bm.parent.on('click', event => {
            if (!event.originalEvent.shiftKey) {
                if (event.originalEvent.ctrlKey) {
                    toggleBanMap(bm, redName, 'red');
                } else if (event.originalEvent.altKey) {
                    toggleProtectMap(bm, redName, 'red');
                } else {
                    togglePickMap(bm, redName, 'red');
                }
            } else {
                resetMap(bm);
            }
        ...
        bm.parent.on('contextmenu', event => {
            if (!event.originalEvent.shiftKey) {
                if (event.originalEvent.ctrlKey) {
                    toggleBanMap(bm, blueName, 'blue');
                } else if (event.originalEvent.altKey) {
                    toggleProtectMap(bm, blueName, 'blue');
                } else {
                    togglePickMap(bm, blueName, 'blue');
                }
            } else {
                resetMap(bm);
            }
...
const togglePickMap = (bm, teamName, color) => {
    if (bm.parent.hasClass('picked')) {
        resetMap(bm);
    } else {
        pickMap(bm, teamName, color);
    }
};

const toggleBanMap = (bm, teamName, color) => {
    if (bm.parent.hasClass('banned')) {
        resetMap(bm);
    } else {
        banMap(bm, teamName, color);
    }
};

const toggleProtectMap = (bm, teamName, color) => {
    if (bm.parent.hasClass('protected')) {
        resetMap(bm);
    } else {
        protectMap(bm, teamName, color);
    }
};

const pickMap = (bm, teamName, color) => {
    if (bm.parent.hasClass('banned')) return;  // Prevent picking a banned map
    if (bm.parent.hasClass('picked')) return;  // Prevent picking a picked map
    if (bm.parent.hasClass('protected')) {
        resetMap(bm);  // Allow picking a protected map, but first reset it
    }
    ...
    if (bm.beatmap.mods.includes('TB')) {
        document.cookie = `lastPick=;path=/`;
        bm.parent.addClass(`picked`).removeClass('banned red blue protected');
        bm.picked_by_label.text('Tiebreaker').addClass(`picked`).removeClass('banned red blue protected');
    }
    else {
        document.cookie = `lastPick=${bm.id}-${color.toLowerCase()};path=/`;
        bm.parent.addClass(`picked ${color}`).removeClass(`banned protected ${opposite_team(color)}`);
        bm.picked_by_label.text(`Picked by ${teamName}`).addClass(`picked ${color}`).removeClass(`banned protected ${opposite_team(color)}`);
    }
...
const banMap = (bm, teamName, color) => {
    if (bm.beatmap.mods.includes('TB') || bm.parent.hasClass('protected') || bm.parent.hasClass('picked')) return;
...
const protectMap = (bm, teamName, color) => {
    if (bm.beatmap.mods.includes('TB') || bm.parent.hasClass('banned') || bm.parent.hasClass('picked')) return;

    bm.parent.addClass(`protected ${color}`).removeClass(`picked ${opposite_team(color)}`);
    bm.picked_by_label.text(`Protected by ${teamName}`).addClass(`protected ${color}`).removeClass(`picked ${opposite_team(color)}`);
    bm.blink_overlay.css('animation', 'none');
    bm.mod_icon.addClass('protected');
    selectedMaps.push(bm.beatmapID);
}
...
const resetMap = bm => {
    ...
    bm.parent.removeClass('banned picked protected red blue');
    bm.picked_by_label.text('').removeClass('banned picked protected red blue');
    bm.blink_overlay.css('animation', 'none');
    bm.mod_icon.removeClass('banned protected');
