  ...
  let checkValid = () => {
    ...
    if (match) {
      ...
      if (mapid == parsedBeatmapID) {
        ...
        if (nameRed && nameBlue) {
          $('#picked_by').css('opacity', 1).addClass(cookieValue[1]).removeClass(opposite_team(cookieValue[1]));
...
socket.onmessage = async event => {
  ...
  if (update_stats) {
    ...
    $('#title').text(`${data.menu.bm.metadata.title}`);
  ...
  if (chatLen !== data.tourney.manager.chat.length) {
    ...
    if (chatLen === 0 || (chatLen > 0 && chatLen > current_chat_len)) {
      // Clear the chat container only if it's the first update or if the chat length has decreased
      $('#chat').html('');
      chatLen = 0;
    }
...
const socket2 = new ReconnectingWebSocket('ws://' + location.host + '/websocket/v2');

let mappool2;
(async () => {
  $.ajaxSetup({ cache: false });
  let stage = await $.getJSON('../_data/beatmaps.json');
  mappool2 = stage.beatmaps;
  if (mappool2) $('#stage_name').html(`${stage.stage} MAPPOOL SHOWCASE`).css('opacity', 1);
  else $('#stage_name').html('').css('opacity', 0);
})();

socket2.onopen = () => { console.log('Successfully Connected'); };
socket2.onclose = event => { console.log('Socket Closed Connection: ', event); socket2.send('Client Closed!'); };
socket2.onerror = error => { console.log('Socket Error: ', error); };

const cache = {};
let update_stats2 = false;
...
socket2.onmessage = async event => {
  ...
  if (mappool2 && cache.md5 !== data.beatmap.checksum) {
  ...
  if (update_stats2) {
    update_stats2 = false;
    const map = mappool2.find(m => m.beatmap_id === data.beatmap.id || m.md5 === cache.md5);
...
const getModStats2 = (cs_raw, ar_raw, od_raw, hp_raw, mods) => {
  let speed = mods.includes('DT') ? 1.5 : mods.includes('HT') ? 0.75 : 1;
  let ar = mods.includes('HR') ? ar_raw * 1.4 : mods.includes('EZ') ? ar_raw * 0.5 : ar_raw;
  let ar_ms = Math.max(Math.min(ar <= 5 ? 1800 - 120 * ar : 1200 - 150 * (ar - 5), 1800), 450) / speed;
